 ...
 *  の責任を負わない．
 *
 *  $Id: jtag_uart.c 1801 2015-03-27 06:34:43Z t_ishikawa $
 */
...
uart_getready(void)
{
	uint32 tmp = sil_rew_iop((void *) (JTAG_UART_PORT1_BASE + JTAG_UART_DATA_OFFSET));
	rx_buf = (uint8) tmp;
	return((tmp & JTAG_UART_DATA_RVALID) != 0U);
}
...
InitHwSerial(void)
{
	 ...
	 *  受信割込み許可
	 */
	sil_wrw_iop((void *) (JTAG_UART_PORT1_BASE + JTAG_UART_CONTROL_OFFSET),
				sil_rew_iop((void *) (JTAG_UART_PORT1_BASE + JTAG_UART_CONTROL_OFFSET))
